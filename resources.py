...
from helpers import Resource, mongo_jsonify, parse_npt, resolve_type, uri_pattern
...
class MediaAsset(Resource):
    ...
    def set_attrs(self):
		...
		for (k,v) in self.request.json.items():
			if k in model.structure['@graph'] and k not in ["dc:identifier","pid","dc:type"]:
				...
				elif model.structure['@graph'][k]==type(u""):
					self.bundle["@graph"][k]=unicode(v)
				if k=="ma:title":
					self.bundle["ititle"]=unicode(v).lower()
				elif model.structure['@graph'][k]==type(2):
					self.bundle["@graph"][k]=int(v)
				elif model.structure['@graph'][k]==type(2.0):
					self.bundle["@graph"][k]=float(v)
				elif type(model.structure['@graph'][k])==type([]):
					self.bundle["@graph"][k]=[]
					for i in v:
						if k=="ma:isMemberOf":
							membership={}
