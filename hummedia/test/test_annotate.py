...
def test_collection_write_access_ta_can_patch_superusers_edit(app, ACCOUNTS):
  ...
  assert req_patch.status_code is 200, "TA with write access could not modify required annotations created by superuser. Status: %d" % req_patch.status_code
...
def test_unauthorized_student_cannot_create_required_annotation(app, ACCOUNTS):
  app.login(ACCOUNTS['SUPERUSER'])
