...
def test_write_access_of_collection_can_add_someone_elses_videos(app, ACCOUNTS):
  app.login(ACCOUNTS['SUPERUSER'])
  c = app.post('/collection', data=json.dumps({}), headers={'Content-Type': 'application/json'})
  v = app.post('/video', data=json.dumps({'dc:creator': ACCOUNTS['STUDENT']['username']}), headers={'Content-Type': 'application/json'})
